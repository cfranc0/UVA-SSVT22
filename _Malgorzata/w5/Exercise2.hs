...
countSurvivors n props fun mutator = sum <$> mapM (countS input props fun ) mutators
...
countS n props fut mutator = length . filter (== False ) <$> mapM (survivor fut props mutator) [1 .. n]
...
survivor fut props mutator input =  all (== False) <$> mutate' mutator props fut input
